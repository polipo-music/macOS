...
def main():
    ...
    for package in packages:
        ...
        if path.suffix in ['.pkg', '.dmg'] and path.stem in chunklist_set:
            chunklist_set.remove(path.stem)
            chunklist_dict[filename] = path.stem + '.chunklist'
        filename.rsplit('.', 1)
    ...
    logging.info('Checking chunklist...')
    for filename, chunklist_filename in chunklist_dict.items():
        if chunklist_filename == 'InstallESDDmg.chunklist':
            # Note: does not match against InstallESDDmg.pkg
            logging.warning(f'Skipped {chunklist_filename}')
            continue
        logging.info(f'Processing {chunklist_filename}...')
        check_chunklist(filename, chunklist_filename)
